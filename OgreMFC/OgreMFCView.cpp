...
OgrePart COgreMFCView::ogrepart;
...
//{
//     FrameListener *mFrameListener= new ExaListener(m_pWindow,m_pCamera,m_pSceneManager);
//
...
void COgreMFCView::CreateRanderWind(void)
{
	ogrepart.m_pRoot->initialise(false);
	NameValuePairList miscParams;
	...
	GetClientRect(&rt);
	ogrepart.m_pWindow = ogrepart.m_pRoot->createRenderWindow("test",rt.Width(),rt.Height(),false,&miscParams);
	//this->m_hWnd;
}
...
void COgreMFCView::OnTimer(UINT_PTR nIDEvent)
{
	...
	ogrepart.m_pRoot->renderOneFrame();
...
void COgreMFCView::OnDraw(CDC* /*pDC*/)
{
	if (!bHasInit)
	{
		...
		ogrepart.SetupResource();
		ogrepart.SetupConfig();
		CreateRanderWind();
		ogrepart.SetupMFC();
...
void COgreMFCView::OnSizing(UINT fwSide, LPRECT pRect)
{
	...
	if (ogrepart.m_pWindow)
	{
		CRect rt;
		GetClientRect(&rt);
		ogrepart.m_pWindow->windowMovedOrResized();

		if (rt.Height() != 0 && ogrepart.m_pCamera != NULL)
		{
			ogrepart.m_pCamera->setNearClipDistance(Real(ogrepart.m_pWindow->getWidth())/Real(ogrepart.m_pWindow->getHeight()));
		}
		ogrepart.m_pCamera->yaw(Radian(0));
	}
